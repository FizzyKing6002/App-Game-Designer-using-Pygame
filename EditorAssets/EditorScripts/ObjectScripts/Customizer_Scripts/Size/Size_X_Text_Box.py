...
class Main:
    def __init__(self):
        ...
        self.position_modifiers = [[0, 1 - 0.05 - 0.025 - 0.825/4], [0, 0.41]]
        # [[pixels, percent of container's size], [pixels, percent of container's size]] -> [x, y]
    ...
    def frame_update(self, global_scripts):
        ...
        if self.broadcast_value:
            ...
            try:
                float(self.text)
                global_scripts.customizer_input(",", self.text, "self.size_modifiers", "[[")
        ...
        if global_scripts.delayed_changed_current_path:
            if hasattr(global_scripts.project_global_scripts, "__editor_attr__selected_size_mod__"):
                self.text = str(
                    global_scripts.project_global_scripts.__editor_attr__selected_size_mod__[0][0])[:5]
            else:
                self.text = ""
