...
class Main:
    def __init__(self):
        ...
        self.object_colour = (158, 255, 108)
    ...
    def frame_update(self, global_scripts):
        if self.startup:
            ...
            priority_counter = 1
            for i in range(len(global_scripts.world)):
                for j in range(len(global_scripts.world[i])):
                    self.generate_object(global_scripts, "Tile", [priority_counter, [j, i], global_scripts.world[i][j], global_scripts.tribe_terrain[i][j]])
                    priority_counter += 1

        if global_scripts.delayed_update_world:
            self.update_map(global_scripts)
    ...
    # Additional methods:
    def update_map(self, global_scripts):
        priority_counter = 1
        for i in range(len(global_scripts.world)):
            for j in range(len(global_scripts.world[i])):
                self.objects[priority_counter + 2].generated_value = [priority_counter, [j, i], global_scripts.world[i][j], global_scripts.tribe_terrain[i][j]]
                self.objects[priority_counter + 2].update(global_scripts)
                priority_counter += 1
